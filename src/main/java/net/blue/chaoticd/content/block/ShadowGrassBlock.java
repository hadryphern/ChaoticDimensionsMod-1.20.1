...
/** Nearly lifeless Shadow grass that spreads only over Shadow soil. */
public final class ShadowGrassBlock extends GrassBlock {
    ...
    public BlockState updateShape(
        ...
        if (direction == Direction.UP && !canRemainGrass(state, level, pos)) {
            return ModBlocks.SHADOW_SOIL.defaultBlockState();
        }
    ...
    public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        if (!canRemainGrass(state, level, pos)) {
            level.setBlockAndUpdate(pos, ModBlocks.SHADOW_SOIL.defaultBlockState());
            return;
        }
        ...
        for (int attempt = 0; attempt < 4; attempt++) {
            ...
            if (level.getBlockState(targetPos).is(ModBlocks.SHADOW_SOIL)
